...
const ProductView = () => {
    ...
    const fetchMostViewedProducts = async () => {
        try {
            ...
            // Filter active products and sort by view count
            const mostViewedProducts = productsData
                .filter(product => product.status === true || product.status === 1)
                .sort((a, b) => {
                    // Get view count from either view_count or viewCount property
                ...
                .slice(0, 8); // Get only the 8 most viewed products
    ...
    if (loading) {
        return (
            <div className="container mx-auto px-4 md:px-8 lg:px-16">
                <h1 className="text-3xl md:text-3xl font-mono text-gray-900 mt-20 mb-12 text-center">SẢN PHẨM XEM NHIỀU NHẤT</h1>
                    ...
                    {products.map(product => (
                        ...
                        >
                            <ProductCard
                                ...
                                viewCount={product.view_count || product.viewCount || 0}
                            />
                        </div>
                    ))}
                </div>
            ) : (
                <div className="text-center text-gray-500 py-6">Không có dữ liệu sản phẩm xem nhiều nhất.</div>
            )}
        </div>
    );
};
